  ...
  try {
    ...
    for (const res of validResults) {
        ...
        if (!error && saved) {
            ...
            // Reuses the article's existing embedding rather than generating a second one.
            if (!saved.image_url && isIcelandicSource(res.feedUrl) && embedding) {
                ...
                const stockImagePath = matchStockImage(embedding, recentlyUsedIds, processed.title);
