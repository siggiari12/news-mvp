...
async function main() {
  ...
  // Include optional Icelandic keywords for cross-language matching
  const descriptions = needsEmbedding.map(img => {
    const parts = [img.description, img.tags.join(' ')];
    if (img.keywords_is && img.keywords_is.length > 0) {
      parts.push(img.keywords_is.join(' '));
    }
    return parts.join(' ');
  });
